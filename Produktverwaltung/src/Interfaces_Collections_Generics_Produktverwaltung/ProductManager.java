...
public class ProductManager {
	...
	/**
	 * addProduct(Product product): Fügt ein Produkt zur Liste hinzu.
	 * @param p
	 */
	 ...
	 * searchProducts(String keyword): Sucht nach Produkten, die das angegebene  	   
      ...
      der gefundenen Produkte zurück.
	 */
	public ArrayList<Product> searchProducts(String keyword){
		ArrayList<Product> results = new ArrayList<Product>();
		for (Product pr : products) {
			if (pr.getName().contains(keyword) | pr.getDescription().contains(keyword) ) {
				// einer Ergebnisliste hinzugefügt werden
				results.add(pr);
			}
		}
		// und gibt eine Liste der gefundenen Produkte zurück
		return results;
	}
